...
| VG-17 | Ledger Protocol | Replayable execution authority over the shared-folder substrate: intent, policy, vote/quorum, lease, execution proof, verification, and close records over `execution_ledger.jsonl`. | `LEDGER_PROTOCOL.md`, `TUI_HARNESS.md`, `HIVE_WORKING_METHOD.md`, `CHAIR_RUNTIME_SPEC.md` | First slice in `hivemind/workloop.py`, `hive ledger`, and TUI ledger view; TODO tracks protocol artifacts, supervisor, bypass quorum, and replay. |
| VG-18 | Prompt/Log AIOS UX | Future user surface hides directories, run folders, artifact paths, and app chrome. The normal workflow is prompt input plus live logs, decisions, blocked gates, and outcomes; richer observability belongs in MemoryOS's neural map. | `NORTHSTAR.md`, `TUI_HARNESS.md`, `LEDGER_PROTOCOL.md`, `ui_future.md` | TUI/CLI are transitional operator/debug surfaces. TODO should move toward prompt-first summaries, MemoryOS-consumable live read models, and AIOS-style operation rather than file browsing. |
