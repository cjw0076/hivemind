...
| VG-15 | Layered Chair Runtime | Split the chair/header into deterministic dispatch, verification, working agents, referee, North-Star auditor, and conflict reviewer so coordination is not a monolithic LLM bottleneck. | `HIVE_MIND_GAPS.md` section "Header Role Decomposition and Per-Layer Provider Selection", `OPERATOR_METHOD_PROFILE.md`, `PROVIDER_MODELS.md`, `HIVE_WORKING_METHOD.md` | Planned in `TODO.md` Layered Chair Runtime; L0/L1 should be code/local-first, L2/L3/L5 provider-heterogeneous, and L4 long-context/event-triggered. |
