...
graph TD
  ...
  V1[MemoryOS]
  V2[CapabilityOS]
  V3[Hive Mind Harness CLI/Prompt-Log]
  V4[Provider Harness]
...
| VG-03 | `hive` Harness CLI/Prompt-Log | Operational blackboard that turns tasks into artifacts for agents/providers. | `HIVE_WORKING_METHOD.md`, `make_production.md`, `hive_mind2.md`, `docs/split/memoryOS/part-008-external-agent.md`; `tui.md` is now archived input. | `hivemind/hive.py`, `hivemind/harness.py`, `hivemind/live.py`; `TODO.md` Now, Harness Runtime, and Production Hardening. |
...
| VG-10 | MemoryOS Neural Map UI | Long-term observability surface owned by MemoryOS: agent turns, authority gates, task flow, claims, evidence, disagreements, memory nodes, and decisions as a graph/map rather than terminal panels or file paths. | `memoryOS.md`, `ui_future.md`, `docs/image.png`, `docs/split/memoryOS/part-011-desktop-4.md`, `NORTHSTAR.md` | Deferred until Hive run artifacts, ledger/protocol read models, and MemoryOS review state are stable; `TODO.md` API And UI. |
...
| VG-13 | Production Packaging | Installable local-first `hive` product wrapping MemoryOS, CapabilityOS, MCP, providers, and local runtime. | `make_production.md`, `hive_mind2.md`, `HIVE_WORKING_METHOD.md`, `PROVIDER_HARNESS_GUIDE.md` | `hive init`, `hive doctor`, `hive local status/setup`; `TODO.md` Harness Runtime, Production Hardening, and Security/Product. |
...
| VG-17 | Ledger Protocol | Replayable execution authority over the shared-folder substrate: intent, policy, vote/quorum, lease, execution proof, verification, and close records over `execution_ledger.jsonl`. | `LEDGER_PROTOCOL.md`, `HIVE_WORKING_METHOD.md`, `CHAIR_RUNTIME_SPEC.md` | First slice in `hivemind/workloop.py`, `hive ledger`, and live/inspect ledger summaries; TODO tracks protocol artifacts, supervisor, bypass quorum, and replay. |
| VG-18 | Prompt/Log AIOS UX | User surface hides directories, run folders, artifact paths, and app chrome. The normal workflow is prompt input plus live logs, decisions, blocked gates, and outcomes; richer observability belongs in MemoryOS's neural map. | `NORTHSTAR.md`, `LEDGER_PROTOCOL.md`, `ui_future.md` | `hive live` is the default surface. Legacy terminal cockpit code has been removed; TODO should move toward prompt-first summaries, MemoryOS-consumable live read models, and AIOS-style operation rather than file browsing. |

## Source Families

| Family | Files | Use when |
| --- | --- | --- |
| Current execution | `HIVE_WORKING_METHOD.md`, `PROVIDER_HARNESS_GUIDE.md`, `LOCAL_LLM_WORKERS.md`, `make_production.md`, `hive_mind2.md`, `cli_help.md` | Changing `hive`, provider invocation, onboarding, local workers. |
