...
| VG-03 | `mos` Harness CLI/TUI | Operational blackboard that turns tasks into artifacts for agents/providers. | `tui.md`, `TUI_HARNESS.md`, `make_production.md`, `docs/split/memoryOS/part-008-external-agent.md` | `memoryos/mos.py`, `memoryos/harness.py`, `memoryos/tui.py`; `TODO.md` Now and Harness Runtime. |
| VG-04 | Provider Harness | Claude, Codex, Gemini, hosted APIs, and future providers behind one artifact protocol. | `PROVIDER_HARNESS_GUIDE.md`, `cli_help.md`, `PROVIDER_MODELS.md`, `make_production.md` | `mos doctor`, `mos invoke`; `TODO.md` Harness Runtime and Agent Harness. |
| VG-05 | Local LLM Runtime | Cheap local workers for classify, extract, compress, summarize, draft, review. | `local_llm_use.md`, `localllm.md`, `LOCAL_LLM_WORKERS.md`, `LOCAL_LLM_INVENTORY.md` | `memoryos/local_workers.py`, `mos local status/setup`; `TODO.md` Local LLM Workers. |
| VG-06 | Agent Society | Observable agent profiles, reviews, feedback, routing proposals, prompt mutation proposals. | `agent_society.md`, `final.md`, `ROADMAP.md` phase 2.6 | `TODO.md` Agent Society; not auto-adaptive yet. |
...
| VG-10 | Desktop / Visual Board | Later cockpit: import center, ask memory, graph explorer, draft review, settings. | `memoryOS.md`, `ui_future.md`, `docs/image.png`, `docs/split/memoryOS/part-011-desktop-4.md` | Deferred until `mos` artifacts and review state are stable; `TODO.md` API And UI. |
| VG-11 | Import Parser Substrate | Official exports and local files become durable normalized nodes with provenance. | `memoryOS.md`, `EXPORT_PARSERS.md`, `MEMORYOS_MVP.md`, `docs/split/memoryOS/part-007-parser-and-mcp-workflow.md` | `memoryos/cli.py`; `TODO.md` Parser Work. |
...
| VG-13 | Production Packaging | Installable local-first `mos` product wrapping MemoryOS, CapabilityOS, MCP, providers, and local runtime. | `make_production.md`, `TUI_HARNESS.md`, `PROVIDER_HARNESS_GUIDE.md` | `mos init`, `mos doctor`, `mos local status/setup`; `TODO.md` Harness Runtime and Security/Product. |

## Source Families

| Family | Files | Use when |
| --- | --- | --- |
| Current execution | `TUI_HARNESS.md`, `PROVIDER_HARNESS_GUIDE.md`, `LOCAL_LLM_WORKERS.md`, `make_production.md`, `cli_help.md` | Changing `mos`, provider invocation, onboarding, local workers. |
